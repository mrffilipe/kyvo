...
| `@kyvo-client/client` | workspace | OIDC (PKCE), session, JWT claims, `users.getMe()`, token refresh |
...
The app depends on `@kyvo-client/client` via `file:../../../sdk/typescript/@kyvo-client/client`. Build the SDK first when working on both packages:

```bash
cd ../../../sdk/typescript && npm install && npm run build
```

During `npm run dev`, Vite resolves `@kyvo-client/client` to the SDK sources (`vite.config.ts` alias) so you do not need to rebuild after every SDK change.
