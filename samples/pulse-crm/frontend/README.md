...
| `@kyvo-client/client` | npm `^1.0.2` | OIDC (PKCE), session, JWT claims, `users.getMe()`, token refresh |
...
The app consumes `@kyvo-client/client` from npm (`npm install` resolves `^1.0.2`).
