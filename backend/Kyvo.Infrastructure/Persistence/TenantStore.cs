...
public sealed class TenantStore : ITenantStore<TenantInfoAdapter>
{
    ...
    public TenantStore(ApplicationDbContext context, IDistributedCache distributedCache, IOptions<RedisOptions> redisOptions)
    {
        _context = context;
    ...
    public async Task<IReadOnlyList<TenantInfoAdapter>> GetAllAsync(CancellationToken ct = default)
    {
    ...
    public async Task<TenantInfoAdapter?> GetByIdentifierAsync(string identifier, CancellationToken ct = default)
    {
        ...
        var tenant = await _context.Tenants
            .AsNoTracking()
            .FirstOrDefaultAsync(x => x.Key.Value == normalized && x.IsActive, ct);
