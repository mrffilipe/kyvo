...
public sealed class AuthSessionRepository : IAuthSessionRepository
{
    ...
    public async Task<IReadOnlyList<AuthSession>> ListActiveByUserIdAndTenantIdForUpdateAsync(
        Guid userId,
        Guid tenantId,
        CancellationToken ct = default)
    {
        return await _context.AuthSessions
            .Where(x => x.UserId == userId && x.TenantId == tenantId && x.Status == SessionStatus.Active)
            .ToListAsync(ct);
    }
}
