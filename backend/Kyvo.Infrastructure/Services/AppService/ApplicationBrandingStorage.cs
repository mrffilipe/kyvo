...
public sealed class ApplicationBrandingStorage : IApplicationBrandingStorage
{
    ...
    public ApplicationBrandingStorage(IWebHostEnvironment environment)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new DomainValidationException(ApplicationErrorMessages.Application.BrandingLogoFileRequired);
        }

        var normalizedContentType = NormalizeContentType(contentType, fileName);
        if (!AllowedContentTypes.Contains(normalizedContentType))
        {
            throw new DomainValidationException(ApplicationErrorMessages.Application.BrandingLogoFileTypeNotAllowed);
        }

        if (content.CanSeek && content.Length > MaxLogoBytes)
        {
            throw new DomainValidationException(ApplicationErrorMessages.Application.BrandingLogoFileTooLarge);
        }
        ...
        if (fileStream.Length > MaxLogoBytes)
        {
            fileStream.Close();
    ...
    public Task DeleteLogoAsync(Guid applicationId, CancellationToken cancellationToken = default)
    {
