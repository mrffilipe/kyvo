...
public sealed class TenantRole : TenantEntity
{
    ...
    private void SetDetails(string name, string? description)
    {
        ...
        {
            throw new DomainValidationException(DomainErrorMessages.TenantRole.NAME_REQUIRED);
        }

        var normalizedName = name.Trim();
        if (normalizedName.Length > 120)
        {
            throw new DomainValidationException(DomainErrorMessages.TenantRole.NAME_MAX_LENGTH);
        }

        var normalizedDescription = string.IsNullOrWhiteSpace(description) ? null : description.Trim();
        if (normalizedDescription?.Length > 500)
        {
            throw new DomainValidationException(DomainErrorMessages.TenantRole.DESCRIPTION_MAX_LENGTH);
        }
